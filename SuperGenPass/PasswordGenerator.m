...
- (BOOL)isValidPassword:(NSString *)password {
  NSRegularExpression *lowerCasePattern = [NSRegularExpression regularExpressionWithPattern:@"[a-z]" options:0 error:nil];
  NSRegularExpression *upperCasePattern = [NSRegularExpression regularExpressionWithPattern:@"[A-Z]" options:0 error:nil];
  NSRegularExpression *digitPattern = [NSRegularExpression regularExpressionWithPattern:@"[\\d]" options:0 error:nil];
  NSRange range = NSMakeRange(0, password.length);
  
  return [lowerCasePattern rangeOfFirstMatchInString:password options:0 range:range].location == 0 &&
         [upperCasePattern rangeOfFirstMatchInString:password options:0 range:range].location != NSNotFound &&
         [digitPattern rangeOfFirstMatchInString:password options:0 range:range].location != NSNotFound;
}
